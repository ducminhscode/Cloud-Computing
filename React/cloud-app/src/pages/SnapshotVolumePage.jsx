...
export default function SnapshotVolumePage() {
    ...
    const fetchSnapshots = async () => {
        try {
            setLoading(true);
            const res = await Apis.get(endpoints['snapshot-volume'], { headers });
            const volumeSnapshots = res.data.snapshots.filter(snap => snap.volume_id); // Lọc snapshot của volume
            setSnapshots(volumeSnapshots);
        } catch (error) {
    ...
    const handleCreateSnapshot = async () => {
        try {
            const payload = { ...newSnapshot, project_id: projectId };
            await Apis.post(endpoints["snapshot-volume"], payload, { headers });
            alert("Tạo snapshot volume thành công");
            setNewSnapshot({ name: "", description: "", volume_id: "" });
            fetchSnapshots();
        } catch (error) {
            alert("Không thể tạo snapshot volume");
        }
    };
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            <h1 className="text-3xl font-bold mb-6">Snapshot Volume</h1>
            <div className="bg-white p-6 rounded-lg shadow mb-8">
                <h2 className="text-xl font-semibold mb-4">Tạo Snapshot Volume</h2>
                <input
                    type="text"
            ...
            <div className="bg-white p-6 rounded-lg shadow">
                <h2 className="text-xl font-semibold mb-4">Danh sách Snapshot Volume</h2>
                {loading ? (
                    <p>Đang tải...</p>
                ) : (
